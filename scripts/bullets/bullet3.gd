...
func _fixed_process(delta):
	if (movem.finished):
		...
		if (get_child(1).get_overlapping_bodies()[0].get_collision_mask() == 12):
			get_child(1).get_overlapping_bodies()[0].bar.takeDamage(50+0.8*get_parent().player.AD, 1, direction)
			get_child(1).get_overlapping_bodies()[0].bar.takeDamage(1.0*get_parent().player.AP, 2, direction)
