...
func speedConv(Speed):
	return speedK*sqrt(Speed)

func setSpeed(Speed):
	speedBase = Speed
	speed = (speedBase + speedAdd)*speedMul
	get_parent().movem.setSpeed(speedConv(speed))
...
func takeHeal(heal, nature, dir):
	var DamageScene = load("res://scenes/UI/DamageLabel.xscn")
	var Damage = DamageScene.instance()
	get_parent().get_parent().add_child(Damage)
	Damage.set_pos(self.get_parent().get_pos())
	if nature == 0 :
		...
		if (curHp + heal > maxHp):
			heal -= (maxHp - curHp)
			if (curHp+barrier > maxHp):
				heal = heal*100/(100+(curHp+barrier-maxHp))
				barrier += heal
			else :
				barrier += heal
			heal += (maxHp - curHp)
...
func _process(delta):
	...
	if (time >= 1):
		time = 0.0
		print("Spd = ", speed, "Arm = ", armor, "Shi = ", shield, "AP = ", AP, "AD = ", AD)
...
func _draw():
	...
	draw_rect (Rect2(Vector2(-52, barPos.y-2), Vector2(104, 14)), Color(178.0/255, 178.0/255, 178.0/255, 1.0))
	draw_rect (Rect2(Vector2(-50, barPos.y), Vector2(100, 10)), Color(117.0/255, 117.0/255, 117.0/255, 0.8))
	...
	while indexHP < barrier + curHp :
		if (indexHP%1000 != 0):
			draw_line(Vector2(-50.0 + indexHP/tempMax*100.0, barPos.y ), Vector2(-50.0 + indexHP/tempMax*100.0, barPos.y + 5), Color(117.0/255, 117.0/255, 117.0/255, 0.8), 1)
		else:
