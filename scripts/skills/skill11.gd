...
func preSetup (i) :
	...
	get_parent().player.bar.takeBuff(30, 3, (index+1))
	get_parent().player.bar.vampirism += 0.1
	get_parent().player.bar.takeBuff(40, 1, (index+1))
	get_parent().player.bar.takeBuff(40, 2, (index+1))
...
func _fixed_process(delta):
	...
	if (OS.get_ticks_msec()/1000.0 > endTime):
		...
		if (initHealth - curHealth > 0):
			damage += (initHealth - curHealth)*(get_parent().player.bar.armor + get_parent().player.bar.shield)/100.0
		print(damage, (get_parent().player.bar.armor + get_parent().player.bar.shield)/100.0)
		if (damage > 1):
			for i in get_child(0).get_overlapping_bodies():
				if (i.get_collision_mask() == 12):
					...
					get_parent().player.bar.takeHeal(newDamage*(get_parent().player.bar.vampirism), 0, self.get_pos() - (i.get_pos() + i.get_parent().get_pos()))
		initHealth = float(get_parent().player.bar.curHp + get_parent().player.bar.barrier)
